...
public class AuthenticatedSupervisorApplicationService {
    ...
    /**
     * 执行正式认证的 Supervisor 多Agent调用（旧方法，保持兼容）。
     * <p>
     * 委托新方法并使用空 threadId。
     *
    ...
    public AuthenticatedSupervisorRunResult invoke(UserSession session, String supervisorName, String message) {
        return invoke(session, supervisorName, message, Optional.empty());
    }

    /**
     * 执行正式认证的 Supervisor 多Agent调用，支持可选 threadId。
     * <p>
     * 执行流程：
     * 1. 基础校验
     ...
     * 7. 调用 SupervisorEngine.executeThread
     * 8. 保存稳定状态
     * 9. 返回结果
     *
     * @param session            已验证的用户会话
    ...
    /**
     * 在 Lease 保护下执行 Supervisor 调用。
     * <p>
     * 包含：挂起检查、执行、保存、返回。
     */
    private AuthenticatedSupervisorRunResult executeWithLease(
            UserSession session,
        ...
        log.info("AuthenticatedSupervisor invoke: runId={}, threadId={}, supervisor={}, userId={}, newThread={}",
                runId, threadId, supervisorName, session.userId(), previousState.isEmpty());

        // ---- 执行 ----
        ThreadExecutionOutcome outcome = supervisorEngine.executeThread(definition, rootTask, context, previousState);
        ...
        if (outcome.conversationState().isPresent()) {
            try {
                ...
                log.info("Supervisor thread memory saved: runId={}, threadId={}, userId={}",
                        runId, threadId, session.userId());
            } catch (AgentFrameworkException e) {
                log.error("Supervisor thread memory save failed: runId={}, threadId={}, errorCode={}",
