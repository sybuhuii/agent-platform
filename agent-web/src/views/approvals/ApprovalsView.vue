...
import type { PendingApprovalSummaryResponse } from '@/types'
...
const rejectTarget = ref<PendingApprovalSummaryResponse | null>(null)
const rejectComment = ref('')
const rejecting = ref<string | null>(null)

async function handleApprove(item: PendingApprovalSummaryResponse) {
  approving.value = item.approvalId
  error.value = ''
  try {
    await approvalApi.decideAndResume(item.runId, item.approvalId, 'APPROVE', '')
...
function openRejectDialog(item: PendingApprovalSummaryResponse) {
...
async function handleReject() {
  if (!rejectTarget.value || !rejectComment.value.trim()) {
    error.value = '请输入拒绝原因'
    return
  }
  rejecting.value = rejectTarget.value.approvalId
  error.value = ''
  try {
    await approvalApi.decideAndResume(
      rejectTarget.value.runId,
      rejectTarget.value.approvalId,
      'REJECT',
...
function riskLevelColor(level: string): string {
  switch (level) {
    ...
    case 'LOW': return 'text-[var(--muted-foreground)]'
...
<template>
  <div class="p-6 max-w-4xl mx-auto">
    <h1 class="text-xl font-semibold mb-4">人机审批</h1>
    ...
    <div v-if="approvalsQuery.isLoading.value" class="text-sm text-[var(--muted-foreground)]">加载中...</div>

    <div v-else-if="!approvalsQuery.data.value?.length" class="text-sm text-[var(--muted-foreground)] py-8 text-center">
      暂无待审批项
    </div>
      ...
      >
        <div class="flex items-center justify-between mb-2">
          <div class="flex items-center gap-2">
            <span class="font-medium text-sm">{{ item.operationName }}</span>
            <span class="text-xs px-1.5 py-0.5 rounded" :class="riskLevelColor(item.riskLevel)">
              {{ item.riskLevel }}
            </span>
          </div>
        ...
        <p class="text-sm text-[var(--muted-foreground)] mb-3">{{ item.reason }}</p>
        ...
        <div class="flex items-center gap-2">
          <button
            ...
            :disabled="approving === item.approvalId || rejecting === item.approvalId"
            @click="handleApprove(item)"
          >
            {{ approving === item.approvalId ? '处理中...' : '通过' }}
          </button>
          <button
    ...
    <!-- 拒绝原因 Dialog — 使用 Reka UI Dialog -->
    <Dialog :open="rejectDialogOpen" @update:open="rejectDialogOpen = $event">
      <template #title>拒绝审批</template>
      <template #description>请输入拒绝原因（必填）</template>
      <template #content>
        <div class="space-y-3">
          <div v-if="rejectTarget" class="text-sm text-[var(--muted-foreground)]">
            操作：{{ rejectTarget.operationName }}
          </div>
          <div>
            <label for="reject-comment" class="block text-sm text-[var(--muted-foreground)] mb-1">拒绝原因</label>
            <textarea
              id="reject-comment"
          ...
          <div class="flex justify-end gap-2">
            ...
            <button
              ...
              :disabled="!rejectComment.trim() || rejecting !== null"
              @click="handleReject"
            >
              {{ rejecting ? '处理中...' : '确认拒绝' }}
            </button>
